"""v4 full-dataset prep, RGB composites instead of SWIR, otherwise identical to v2.

Tests whether the pretrained VLM's in-distribution RGB representations outperform
our literature-recommended SWIR2-SWIR1-NIR composite. The SmallMinesDS paper's own
Table I shows pretrained U-Net on plain RGB (IoU 0.754) beats from-scratch U-Net on
RGB+ (IoU 0.651), suggesting "pretraining distribution matters more than band choice."
v4 tests whether the same holds for our 450M VLM.

ONLY CHANGE from v2: composite uses bands (2, 1, 0) = (red, green, blue) instead of
(9, 8, 6) = (SWIR2, SWIR1, NIR). Same 2-98 percentile stretch. Everything else -
enriched descriptions, multi-task grounding, fixed seeding, stratification, is
identical to v2 so the ONLY experimental variable is the composite band choice.

Output path: `/galamsey/data/v4/`

Usage:
    uv run modal run scripts/prepare_v4_rgb_dataset_modal.py
"""
...
MODAL_V4_DATA_DIR = f"{MODAL_MOUNT_POINT}/data/v4"
...
app = modal.App("galamsey-v4-rgb-prep")
...
def prepare() -> dict:
    ...
    DESCRIPTION_PROMPT = (
        "You are analyzing a Sentinel-2 RGB satellite image "
        "of southwestern Ghana. Describe any signs of illegal small-scale gold mining "
        "(galamsey) activity: exposed soil, excavation pits, sediment plumes, "
    ...
    data_dir = Path(MODAL_V4_DATA_DIR)
    images_dir = data_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("GalamseyWatch, v4 RGB full dataset prep on Modal")
    print("=" * 60)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        def process_patch(entry, idx, is_train):
            ...
            composite = compose_rgb(bands)
            png_name = f"v4_{idx:06d}.png"
    ...
    # ---------- write JSONL files ----------
    output_files = [
        ("galamsey_v4_description_train.jsonl", description_train),
        ("galamsey_v4_description_eval.jsonl", description_eval),
        ("galamsey_v4_grounding_train.jsonl", grounding_train),
        ("galamsey_v4_grounding_eval.jsonl", grounding_eval),
        ("galamsey_v4_multitask_train.jsonl", multitask_train),
    ]
...
def main() -> None:
    ...
    for key, value in result.items():
        print(f"  {key}: {value}")
    print("\nNext step:")
    print("  uv run leap-finetune configs/galamsey_v4_modal.yaml")
