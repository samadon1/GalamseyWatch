"""v7 (1.6B + augmented RGB+SWIR) pixel-IoU eval, epoch-1 checkpoint.

Uses the epoch-1 checkpoint from the interrupted v7 training run. Same
multi-image (RGB+SWIR) format as v5 eval.

Usage:
    uv run modal run scripts/eval_v7_pixel_iou_modal.py
"""
...
V7_CHECKPOINT = (
    "/galamsey/lfm2.5-VL-1.6B-vlm_sft-galamsey_v-all-lr1em05-w0p0-no_lora-20260416_130827"
    "/checkpoint-4430"
)
...
app = modal.App("galamsey-v7-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    print(f"  {V7_CHECKPOINT}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    processor = AutoProcessor.from_pretrained(V7_CHECKPOINT, trust_remote_code=True)
    ...
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
    print(f"  {'v7 (LFM2.5-VL-1.6B, RGB+SWIR, 3ep)':<35} {iou:>8.4f} {precision:>11.4f} {recall:>8.4f} {sdc:>8.4f}")
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
