"""v10 (450M + 8x aug + photometric jitter, RGB+SWIR) pixel-IoU eval, final-epoch checkpoint.

Evaluates the v10-e5 (step 59065) checkpoint on the full SmallMinesDS test split.
v10 extends v9 with photometric jitter (brightness/contrast/saturation ±20%) on
top of the D4 geometric augmentation, and 5 epochs of training from the base
model instead of 3 epochs from the v7 checkpoint.

Usage:
    uv run modal run scripts/eval_v10_pixel_iou_modal.py
"""
...
V10_CHECKPOINT_E5 = (
    f"{V10_RUN_DIR}/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-e5s59065-20260418_165633"
)
...
app = modal.App("galamsey-v10-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    # ---------- Eval v10-e5 ----------
    v10_results = eval_checkpoint("v10-e5 (step 59065)", V10_CHECKPOINT_E5)

    # v9-e3 numbers from prior eval (paste here so the comparison prints inline).
    # These came from the eval_v9_pixel_iou_modal.py run that produced the
    # metrics-modal table numbers on the landing page (0.332 / 0.592 / 0.499).
    # The other four pixel-level fields were also captured in that eval, paste
    # the exact numbers here from Modal if you want the delta to be precise.
    v9_e3 = {
        "pixel_iou": 0.332, "pixel_precision": 0.0, "pixel_recall": 0.592,
        "pixel_sdc_f1": 0.499, "pixel_accuracy": 0.0,
        "mean_per_patch_iou": 0.0, "patch_accuracy": 0.795,
    }

    print("\n" + "=" * 72)
    print("v9-e3 vs v10-e5 COMPARISON")
    print("=" * 72)
    print(f"  {'Metric':<25} {'v9-e3':>10} {'v10-e5':>10} {'Δ (v10-v9)':>12}")
    print(f"  {'-' * 25} {'-' * 10} {'-' * 10} {'-' * 12}")
    for metric in ["pixel_iou", "pixel_precision", "pixel_recall", "pixel_sdc_f1",
                   "pixel_accuracy", "mean_per_patch_iou", "patch_accuracy"]:
        v9v, v10v = v9_e3[metric], v10_results[metric]
        delta = v10v - v9v
        arrow = "↑" if delta > 0 else ("↓" if delta < 0 else "=")
        print(f"  {metric:<25} {v9v:>10.4f} {v10v:>10.4f} {delta:>+10.4f} {arrow}")

    winner = "v10" if v10_results["pixel_iou"] > v9_e3["pixel_iou"] else "v9"
    print(f"\n  Winner (by pixel IoU): {winner}")

    return {"v10": v10_results, "v9_e3_reference": v9_e3, "winner": winner}


@app.local_entrypoint()
def main() -> None:
    print("Submitting v10 pixel-IoU eval to Modal...")
