"""v9 (450M + 8x aug RGB+SWIR) pixel-IoU eval, final-epoch checkpoint.

Evaluates the v9-e3 (step 17719) checkpoint on the full SmallMinesDS test split.
v9 extends v8 with the full D4 dihedral group augmentation (adds rotations on
top of hflip/vflip).

Usage:
    uv run modal run scripts/eval_v9_pixel_iou_modal.py
"""
...
V9_CHECKPOINT_E3 = (
    f"{V9_RUN_DIR}/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-e2s17719-20260417_004324"
)
...
app = modal.App("galamsey-v9-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    # ---------- Eval v9-e3 ----------
    v9_results = eval_checkpoint("v9-e3 (step 17719)", V9_CHECKPOINT_E3)

    # v8-e3 numbers from prior eval (paste here so the comparison prints inline)
    v8_e3 = {
        "pixel_iou": 0.2948, "pixel_precision": 0.4232, "pixel_recall": 0.4928,
        "pixel_sdc_f1": 0.4553, "pixel_accuracy": 0.9215,
        "mean_per_patch_iou": 0.2081, "patch_accuracy": 0.7607,
    }

    print("\n" + "=" * 72)
    print("v8-e3 vs v9-e3 COMPARISON")
    print("=" * 72)
    print(f"  {'Metric':<25} {'v8-e3':>10} {'v9-e3':>10} {'Δ (v9-v8)':>12}")
    print(f"  {'-' * 25} {'-' * 10} {'-' * 10} {'-' * 12}")
    for metric in ["pixel_iou", "pixel_precision", "pixel_recall", "pixel_sdc_f1",
                   "pixel_accuracy", "mean_per_patch_iou", "patch_accuracy"]:
        v8v, v9v = v8_e3[metric], v9_results[metric]
        delta = v9v - v8v
        arrow = "↑" if delta > 0 else ("↓" if delta < 0 else "=")
        print(f"  {metric:<25} {v8v:>10.4f} {v9v:>10.4f} {delta:>+10.4f} {arrow}")

    winner = "v9" if v9_results["pixel_iou"] > v8_e3["pixel_iou"] else "v8"
    print(f"\n  Winner (by pixel IoU): {winner}-e3")

    return {"v9": v9_results, "v8_e3_reference": v8_e3, "winner": winner}


@app.local_entrypoint()
def main() -> None:
    print("Submitting v9 pixel-IoU eval to Modal...")
