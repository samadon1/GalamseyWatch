"""Phase A.4 head-to-head: action-match accuracy for the RICH-CONTEXT two-layer.

Same models as the baseline (v9-e3 perception + LFM2-2.6B policy) but the LFM2
prompt now also carries `mission_priors` and `neighbor_summary` — the same
scalar context the unified VLM gets in its training prompts. Isolates the
"unified architecture" effect from the "richer context" effect: if the gap
between unified and two-layer collapses here, it was about context, not
architecture; if the gap holds, it was about architecture.

Reads the eval JSONL's user_text (which already contains mission_priors +
neighbor_summary, since it was written for unified-VLM training), splices that
into the LFM2 prompt alongside the perception output. Few-shot examples are
extended to demonstrate using the new fields so LFM2 doesn't ignore them.

Usage:
    cd training && uv run modal run scripts/eval_two_layer_rich_context_modal.py
"""
...
EVAL_JSONL = "data/unified_v2/galamsey_unified_v2_eval.jsonl"
IMAGE_ROOT = "data/unified_v2/images"
PREDICTIONS_OUT = "data/unified_v2/predictions_two_layer_rich_context.jsonl"
...
SYSTEM_PROMPT = (
    ...
    "For each tile, you receive (1) the on-board VLM's bounding boxes + description, (2) the "
    "AOI mission prior (what's expected at this location), and (3) a structured neighbor "
    "summary showing what each adjacent tile decided this pass. Use ALL of these signals — "
    "the priors say what to expect, the neighbors say where active clusters are, and the "
    "perception says what's actually in the pixels. Visual evidence is authoritative: when "
    "a tile is pristine, label discard even if priors and neighbors look active.\n\n"
    "Reply with EXACTLY ONE tool call — no preamble, no alternatives, no questions.\n\n"
    "Five actions:\n"
    "- **discard**: 0 boxes OR description says forest/water/undisturbed/cloud-obscured. Use even when priors and neighbors are active if the in-tile pixels are clean (interior holdout pattern).\n"
    "- **flag_for_review**: 1-2 boxes with low/moderate confidence, ambiguous descriptions, cluster-bracketed but in-tile signal subtle, OR strong signal but budget too tight to spend. Cheap (text-only).\n"
    "- **request_higher_resolution**: small candidate that needs more pixels to confirm.\n"
    "- **request_neighbor_tile**: feature continues off-frame in a specific direction (north/south/east/west).\n"
    "- **downlink_now**: confidence >= 0.85 AND (>=2 boxes OR a single large box) AND description names active galamsey features (pits, sediment plumes, exposed soil, turbid water). When met you MUST call downlink_now — but downgrade to flag_for_review if budget < ~50 KB."
)

# Few-shot examples for the rich-context prompt: each demonstrates how to
# combine VLM perception + mission_priors + neighbor_summary into one decision.
# Without examples, LFM2-2.6B falls back to plain-text action names instead of
# the wrapped <|tool_call_start|>...<|tool_call_end|> format the parser expects.
FEW_SHOT: list[tuple[str, str]] = [
    # 1. Pristine forest in active AOI: priors+neighbors look active, pixels clean -> discard (interior holdout).
    ("Tile e001 at lon=-2.7500, lat=5.6400.\n"
     "Cloud cover: 0%. Captured: 2024-01-15T10:39:00Z.\n\n"
     ...
     "Mission priors: Bibiani: known active galamsey cluster, known-distribution training site\n"
     "Neighbor summary (structured):\n"
     '{"north": {"action": "downlink", "boxes": 6, "scene_hint": "cluster"}, '
     '"south": null, "east": {"action": "flag_for_review", "boxes": 0, "scene_hint": "cluster"}, '
     '"west": {"action": "downlink", "boxes": 4, "scene_hint": "cluster"}}\n\n'
     "Pass budget: 320 KB of 512 KB remaining.\n\n"
     "Choose the appropriate tool.",
     '<|tool_call_start|>[discard(reason="pristine canopy in this tile despite Bibiani prior and active cluster on 3 sides — visual evidence wins")]<|tool_call_end|>'),
    # 2. Clear galamsey + cluster context + budget OK -> downlink_now.
    ("Tile e002 at lon=-2.1000, lat=5.5500.\n"
     "Cloud cover: 0%. Captured: 2024-01-15T10:39:00Z.\n\n"
     ...
     "Mission priors: Pra basin near Bogoso: heavy galamsey, river sediment plumes common\n"
     "Neighbor summary (structured):\n"
     '{"north": {"action": "downlink", "boxes": 4, "scene_hint": "cluster_continuation"}, '
     '"south": {"action": "downlink", "boxes": 1, "scene_hint": "cluster"}, '
     '"east": {"action": "downlink", "boxes": 4, "scene_hint": "cluster"}, '
     '"west": {"action": "discard", "boxes": 0, "scene_hint": "forest"}}\n\n'
     "Pass budget: 350 KB of 512 KB remaining.\n\n"
     "Choose the appropriate tool.",
     '<|tool_call_start|>[downlink_now(reason="4 high-confidence pits with sediment plumes, fully cluster-bracketed in heavy-galamsey AOI, budget OK")]<|tool_call_end|>'),
    # 3. Ambiguous in active AOI -> flag_for_review.
    ("Tile e003 at lon=-0.5500, lat=6.2000.\n"
     ...
     "Mission priors: Atewa: known galamsey encroachment frontier, threatened reserve\n"
     "Neighbor summary (structured):\n"
     '{"north": {"action": "discard", "boxes": 0, "scene_hint": "forest"}, '
     '"south": null, "east": {"action": "flag_for_review", "boxes": 0, "scene_hint": "mixed"}, '
     '"west": null}\n\n'
     "Pass budget: 200 KB of 512 KB remaining.\n\n"
     "Choose the appropriate tool.",
     '<|tool_call_start|>[flag_for_review(reason="single ambiguous small candidate at Atewa frontier, neighbor mixed; not worth bandwidth yet")]<|tool_call_end|>'),
    # 4. Strong signal but budget exhausted -> budget-aware downgrade to flag.
    ("Tile e004 at lon=-2.7500, lat=5.6400.\n"
     "Cloud cover: 0%. Captured: 2024-01-15T10:39:00Z.\n\n"
     "VLM detection:\n- 3 mining-pit candidate bounding box(es) (largest area = 0.140)\n"
     '- Description: "Clear excavation pits with exposed lateritic soil and sediment plume."\n'
     "- Derived confidence: 0.85\n\n"
     "Mission priors: Bibiani: known active galamsey cluster, known-distribution training site\n"
     "Neighbor summary (structured):\n"
     '{"north": {"action": "downlink", "boxes": 7, "scene_hint": "cluster_continuation"}, '
     '"south": null, "east": null, "west": null}\n\n'
     "Pass budget: 9 KB of 512 KB remaining.\n\n"
     "Choose the appropriate tool.",
     '<|tool_call_start|>[flag_for_review(reason="clear galamsey signal but only 9KB budget left — flag preserves the lead for next pass")]<|tool_call_end|>'),
    # 5. Urban tile (built env, SWIR bright is buildings not mining) -> discard.
    ("Tile e005 at lon=-0.2000, lat=5.5500.\n"
     "Cloud cover: 0%. Captured: 2024-01-15T10:39:00Z.\n\n"
     "VLM detection:\n- 0 mining-pit candidate bounding box(es)\n"
     '- Description: "Dense urban built area with red roofs and roads, bright SWIR is rooftops."\n'
     "- Derived confidence: 0.00\n\n"
     "Mission priors: Accra: urban, SWIR brightness is built environment\n"
     "Neighbor summary (structured):\n"
     '{"north": {"action": "discard", "boxes": 0, "scene_hint": "urban"}, '
     '"south": {"action": "discard", "boxes": 0, "scene_hint": "urban"}, '
     '"east": {"action": "discard", "boxes": 0, "scene_hint": "urban"}, '
     '"west": null}\n\n'
     "Pass budget: 480 KB of 512 KB remaining.\n\n"
     "Choose the appropriate tool.",
     '<|tool_call_start|>[discard(reason="Accra urban tile, SWIR brightness is built environment per prior, no mining")]<|tool_call_end|>'),
]
GROUNDING_PROMPT = (
...
app = modal.App("galamsey-two-layer-rich-context-eval")
...
def evaluate() -> dict:
    ...
    def build_tile_prompt(*, tile_id, lon, lat, cloud_cover, captured_at,
                          ...
                          mission_priors, neighbor_summary_json):
        ...
        return (
            ...
            f"Mission priors: {mission_priors}\n"
            f"Neighbor summary (structured):\n{neighbor_summary_json}\n\n"
    ...
    with torch.no_grad():
        for i, row in enumerate(eval_rows):
            ...
            mp_match = re.search(r"Mission priors: (.+?)\n", user_text)
            mission_priors = mp_match.group(1).strip() if mp_match else "(none)"
            ns_match = re.search(r"Neighbor summary \(structured\):\n(\{.*?\n\})\n",
                                 user_text, re.DOTALL)
            neighbor_summary_json = ns_match.group(1).strip() if ns_match else "{}"
            ...
            tile_prompt = build_tile_prompt(
                ...
                mission_priors=mission_priors, neighbor_summary_json=neighbor_summary_json,
            )
    ...
    print("\n" + "=" * 72)
    print(f"TWO-LAYER RICH-CONTEXT accuracy: {n_correct}/{len(eval_rows)} = {accuracy:.4f}")
...
def main() -> None:
    ...
    print(f"\nFinal: two-layer rich-context accuracy={result['accuracy']:.4f} on {result['n_total']} examples.")
