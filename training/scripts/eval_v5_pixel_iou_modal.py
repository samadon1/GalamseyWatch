"""v5 (RGB + SWIR multi-image) pixel-IoU eval on the full SmallMinesDS test split.

Same methodology as v2/v4 evals but feeds TWO images per sample (RGB + SWIR),
matching how v5 was trained.

Usage:
    uv run modal run scripts/eval_v5_pixel_iou_modal.py
"""
...
V5_CHECKPOINT = (
    "/galamsey/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-20260416_091323"
    "/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-e2s2215-20260416_091323"
)
...
app = modal.App("galamsey-v5-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    print(f"  {V5_CHECKPOINT}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    processor = AutoProcessor.from_pretrained(V5_CHECKPOINT, trust_remote_code=True)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        with torch.no_grad():
            for i, entry in enumerate(test_selected):
                ...
                # v5: render BOTH composites → PIL
                rgb_pil = Image.fromarray(compose_rgb(bands))
                swir_pil = Image.fromarray(compose_swir(bands))

                # Build VLM conversation with two images (RGB first, SWIR second)
                messages = [{
                    "role": "user",
                    "content": [
                        {"type": "image", "image": rgb_pil},
                        {"type": "image", "image": swir_pil},
    ...
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
    print(f"  {'v5 (LFM2.5-VL-450M, RGB+SWIR)':<35} {iou:>8.4f} {precision:>11.4f} {recall:>8.4f} {sdc:>8.4f}")
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
