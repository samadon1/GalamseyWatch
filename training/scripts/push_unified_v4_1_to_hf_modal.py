"""Push v4_1 merged checkpoint to samwell/galamsey-unified-v4-1 on HuggingFace.
...
attaches the model card from docs/hf_model_card_unified_v4_1.md, pushes via
push_to_hub.

Usage:
    cd training && uv run modal run scripts/push_unified_v4_1_to_hf_modal.py
"""
...
HF_REPO_ID = "samwell/galamsey-unified-v4-1"
...
image = (
    ...
    .add_local_file(
        str(LOCAL_REPO / "docs" / "hf_model_card_unified_v4_1.md"),
        "/root/MODEL_CARD.md",
    )
)

app = modal.App("galamsey-push-unified-v4_1")
...
def push() -> dict:
    ...
    ckpt_path = Path(MODAL_MOUNT_POINT) / V4_1_CHECKPOINT
