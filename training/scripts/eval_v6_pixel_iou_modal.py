"""v6 (change detection: 2016 vs 2022) pixel-IoU eval.

Feeds the model 2016 + 2022 RGB composites of the SAME location and compares
predicted bboxes against the CHANGE mask (mining in 2022 that wasn't in 2016).

Only evaluates the ~192 test locations where both years have matching splits.

Usage:
    uv run modal run scripts/eval_v6_pixel_iou_modal.py
"""
...
V6_CHECKPOINT = (
    "/galamsey/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-20260416_095638"
    "/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-e2s772-20260416_095638"
)
...
app = modal.App("galamsey-v6-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    # Build paired TEST locations (both years must be test)
    all_patch_nums = sorted(set(pnum for (pnum, _) in split_by_year))
    test_locations = []
    for pnum in all_patch_nums:
        s16 = split_by_year.get((pnum, "2016"))
        s22 = split_by_year.get((pnum, "2022"))
        if s16 == "test" and s22 == "test":
            test_locations.append(pnum)
    print(f"  test locations with matched splits: {len(test_locations)}")

    # Build file index
    with zipfile.ZipFile(zip_path, "r") as zf_scan:
        file_index = {}
        for name in zf_scan.namelist():
    ...
    # ---------- Load v6 checkpoint ----------
    print(f"\n[2/4] Loading v6 checkpoint...")
    print(f"  {V6_CHECKPOINT}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    processor = AutoProcessor.from_pretrained(V6_CHECKPOINT, trust_remote_code=True)
    ...
    # ---------- Main eval loop ----------
    print(f"\n[3/4] Running inference on {len(test_locations)} test location pairs...")
    ...
    n_empty_pred_on_pos = 0
    n_true_positive_class = 0
    n_true_negative_class = 0
    n_false_positive_class = 0
    n_false_negative_class = 0
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        with torch.no_grad():
            for i, pnum in enumerate(test_locations):
                # Check all 4 files exist
                keys_needed = [
                    (pnum, "2016", "image"), (pnum, "2016", "mask"),
                    (pnum, "2022", "image"), (pnum, "2022", "mask"),
                ]
                if any(k not in file_index for k in keys_needed):
                    continue
                ...
                # Ground truth = CHANGE mask (new mining only)
                gt_binary = ((mask_2022 > 0) & (mask_2016 == 0)).astype(np.uint8)

                # Render RGB composites for both years → PIL
                rgb_2016_pil = Image.fromarray(compose_rgb(bands_2016))
                rgb_2022_pil = Image.fromarray(compose_rgb(bands_2022))

                # Build VLM conversation: 2016 image first, 2022 image second
                messages = [{
                    "role": "user",
                    "content": [
                        {"type": "image", "image": rgb_2016_pil},
                        {"type": "image", "image": rgb_2022_pil},
                ...
                if (i + 1) % 50 == 0:
                    ...
                    rate = (i + 1) / elapsed
                    eta_sec = (len(test_locations) - (i + 1)) / rate
                    running_iou = total_tp / max(total_tp + total_fp + total_fn, 1)
                    print(
                        f"  [{i+1:>4}/{len(test_locations)}] "
    ...
    with zipfile.ZipFile(zip_path, "r") as zf_ceil:
        def load_mask_ceil(pnum, year):
            key = (pnum, year, "mask")
            if key not in file_index:
                return None
            with zf_ceil.open(file_index[key]) as fp:
                with rasterio.MemoryFile(fp.read()) as mf:
                    with mf.open() as src:
                        return src.read(1)

        for pnum in test_locations:
            mask_2016 = load_mask_ceil(pnum, "2016")
            mask_2022 = load_mask_ceil(pnum, "2022")
            if mask_2016 is None or mask_2022 is None:
                continue
            # Change mask = new mining only
            gt_binary = ((mask_2022 > 0) & (mask_2016 == 0)).astype(np.uint8)
    ...
    results = {
        "n_test_patches": len(test_locations),
        "n_positives": len(test_pos),
        "n_negatives": len(test_neg),
        "total_inference_time_sec": round(elapsed, 1),
        "mean_inference_time_sec": round(elapsed / len(test_locations), 3),
        ...
        "patch_accuracy": round(
            (n_true_positive_class + n_true_negative_class) / len(test_locations), 4
        ),
        # Output hygiene
    ...
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
    print(f"  {'v6 (LFM2.5-VL-450M, change det)':<35} {iou:>8.4f} {precision:>11.4f} {recall:>8.4f} {sdc:>8.4f}")
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
