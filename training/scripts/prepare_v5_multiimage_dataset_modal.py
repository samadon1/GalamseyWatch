"""v5 full-dataset prep, MULTI-IMAGE (RGB + SWIR composites per patch).

Tests whether feeding both an in-distribution RGB composite AND an out-of-
distribution SWIR false-color composite to the VLM simultaneously improves
over either alone. Each JSONL entry has TWO images in the user content list.

v2 (SWIR only): IoU 0.159
v4 (RGB only):  IoU 0.196
v5 (RGB + SWIR): ???

Three possible outcomes:
  - v5 > v4 → SWIR adds incremental value alongside RGB
  - v5 ≈ v4 → SWIR adds nothing for pretrained VLMs
  - v5 < v4 → two images confuse the model

Output path: `/galamsey/data/v5/`

Usage:
    uv run modal run scripts/prepare_v5_multiimage_dataset_modal.py
"""
...
MODAL_V5_DATA_DIR = f"{MODAL_MOUNT_POINT}/data/v5"
...
app = modal.App("galamsey-v5-multiimage-prep")
...
def prepare() -> dict:
    ...
    data_dir = Path(MODAL_V5_DATA_DIR)
    images_dir = data_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("GalamseyWatch, v5 multi-image (RGB + SWIR) dataset prep on Modal")
    print("=" * 60)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        def process_patch(entry, idx, is_train):
            ...
            # v5: render BOTH composites per patch
            rgb_composite = compose_rgb(bands)
            swir_composite = compose_swir(bands)
            rgb_name = f"v5_rgb_{idx:06d}.png"
            swir_name = f"v5_swir_{idx:06d}.png"
    ...
    # ---------- write JSONL files ----------
    output_files = [
        ("galamsey_v5_description_train.jsonl", description_train),
        ("galamsey_v5_description_eval.jsonl", description_eval),
        ("galamsey_v5_grounding_train.jsonl", grounding_train),
        ("galamsey_v5_grounding_eval.jsonl", grounding_eval),
        ("galamsey_v5_multitask_train.jsonl", multitask_train),
    ]
...
def main() -> None:
    ...
    for key, value in result.items():
        print(f"  {key}: {value}")
    print("\nNext step:")
    print("  uv run leap-finetune configs/galamsey_v5_modal.yaml")
