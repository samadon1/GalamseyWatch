"""v8 production dataset prep, 450M WebGPU model, 4× augmentation, filtered bboxes.
...
Output path: `/galamsey/data/v8/`

Usage:
    uv run modal run scripts/prepare_v8_production_modal.py
"""
...
MODAL_V8_DATA_DIR = f"{MODAL_MOUNT_POINT}/data/v8"
...
app = modal.App("galamsey-v8-production-prep")
...
def prepare() -> dict:
    ...
    def analyze_mask(mask, min_pixels=MIN_PIT_PIXELS):
        ...
        for pid in range(1, n_pits + 1):
            ...
            if pixel_count < min_pixels:
                continue  # v8: skip tiny components
            ys, xs = np.where(pm)
            bboxes.append([
    ...
    # ---------- main ----------
    data_dir = Path(MODAL_V8_DATA_DIR)
    images_dir = data_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("GalamseyWatch, v8 production dataset (4× aug, filtered bboxes)")
    print("=" * 60)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        def process_patch(entry, idx, is_train):
            ...
            for aug_name, aug_fn in variants:
                ...
                rgb = compose_rgb(v_bands)
                swir = compose_swir(v_bands)
                rgb_name = f"v8_rgb_{new_idx:06d}.png"
                swir_name = f"v8_swir_{new_idx:06d}.png"
                Image.fromarray(rgb).save(images_dir / rgb_name)
                Image.fromarray(swir).save(images_dir / swir_name)

                stats = analyze_mask(v_mask, min_pixels=MIN_PIT_PIXELS)
                # Count how many components were filtered
        ...
        print(f"\n  Processing train ({len(train_selected)} patches × 4 augmentations)...")
        idx = 0
        ...
        for name, samples in [
            ("galamsey_v8_description_train.jsonl", description_train),
            ("galamsey_v8_description_eval.jsonl", description_eval),
            ("galamsey_v8_grounding_train.jsonl", grounding_train),
            ("galamsey_v8_grounding_eval.jsonl", grounding_eval),
            ("galamsey_v8_multitask_train.jsonl", multitask_train),
        ]:
            path = data_dir / name
        ...
        return {
            ...
            "augmentation": "4x (orig+hflip+vflip+hvflip)",
...
def main():
    print("Submitting v8 production dataset prep to Modal...")
