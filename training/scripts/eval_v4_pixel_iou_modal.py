"""v4 (RGB) pixel-IoU eval on the full SmallMinesDS test split.

Same methodology as eval_v2_pixel_iou_modal.py but using:
  - v4 checkpoint (trained on RGB composites)
  - RGB composite rendering (bands 2,1,0 instead of 9,8,6)
  - RGB-appropriate grounding prompt

Usage:
    uv run modal run scripts/eval_v4_pixel_iou_modal.py
"""
...
V4_CHECKPOINT = (
    "/galamsey/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-20260416_074719"
    "/lfm2.5-VL-450M-vlm_sft-galamsey_v-all-lr2em05-w0p0-no_lora-e3s2217-20260416_074719"
)

# v4 uses RGB bands (2,1,0) instead of SWIR (9,8,6)
BAND_RED = 2
BAND_GREEN = 1
BAND_BLUE = 0
...
app = modal.App("galamsey-v4-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    print(f"  {V4_CHECKPOINT}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    processor = AutoProcessor.from_pretrained(V4_CHECKPOINT, trust_remote_code=True)
    ...
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
    print(f"  {'v4 (LFM2.5-VL-450M, RGB)':<35} {iou:>8.4f} {precision:>11.4f} {recall:>8.4f} {sdc:>8.4f}")
    print(f"  {'-' * 35} {'-' * 8} {'-' * 11} {'-' * 8} {'-' * 8}")
