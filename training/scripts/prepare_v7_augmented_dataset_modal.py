"""v7 full-dataset prep, RGB+SWIR multi-image WITH data augmentation.

For training on LFM2.5-VL-1.6B (3.5× larger model) with augmented data.

Changes from v5:
  1. **Horizontal flip augmentation**, each patch gets a second variant with
     both composites horizontally flipped. Mask is flipped too, so bboxes and
     descriptions are regenerated from the transformed mask (correct spatial
     references). Effectively 2× the training data.
  2. Output path: `/galamsey/data/v7/`
  3. PNG prefix: `v7_`

Everything else (RGB+SWIR multi-image, enriched descriptions, multi-task mix,
fixed seeding, stratification) is identical to v5.

Usage:
    uv run modal run scripts/prepare_v7_augmented_dataset_modal.py
"""
...
MODAL_V7_DATA_DIR = f"{MODAL_MOUNT_POINT}/data/v7"
...
app = modal.App("galamsey-v7-augmented-prep")
...
def prepare() -> dict:
    ...
    # v5: prompts reference BOTH images
    DESCRIPTION_PROMPT = (
    ...
    # v4 KEY CHANGE: RGB composite (bands 2, 1, 0) instead of SWIR (9, 8, 6)
    BAND_RED = 2
    BAND_GREEN = 1
    BAND_BLUE = 0
    ...
    def make_vlm_message_multi(rgb_filename, swir_filename, user_text, assistant_text):
        """v5: two images per user turn (RGB first, SWIR second)."""
        return {
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "image", "image": rgb_filename},
                        {"type": "image", "image": swir_filename},
    ...
    data_dir = Path(MODAL_V7_DATA_DIR)
    images_dir = data_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("GalamseyWatch, v7 augmented multi-image (RGB + SWIR) dataset prep on Modal")
    print("=" * 60)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        def process_patch(entry, idx, is_train, augment_train=True):
            """Process one patch. For training, also emit a horizontally-flipped variant."""
            ...
            variants = [("orig", bands, mask)]
            if is_train and augment_train:
                # Horizontal flip: flip along the W axis (axis=2 for bands CxHxW, axis=1 for mask HxW)
                bands_hflip = np.flip(bands, axis=2).copy()
                mask_hflip = np.flip(mask, axis=1).copy()
                variants.append(("hflip", bands_hflip, mask_hflip))

            new_idx = idx
            for variant_name, v_bands, v_mask in variants:
                rgb_composite = compose_rgb(v_bands)
                swir_composite = compose_swir(v_bands)
                rgb_name = f"v7_rgb_{new_idx:06d}.png"
                swir_name = f"v7_swir_{new_idx:06d}.png"
                Image.fromarray(rgb_composite).save(images_dir / rgb_name)
                Image.fromarray(swir_composite).save(images_dir / swir_name)

                # analyze_mask on the TRANSFORMED mask → bboxes have correct coordinates
                stats = analyze_mask(v_mask)
                ...
                grd_msg = make_vlm_message_multi(rgb_name, swir_name, GROUNDING_PROMPT, json.dumps(grd_payload))
            ...
            return new_idx  # return next available idx

        idx = 0
        for entry in train_selected:
            idx = process_patch(entry, idx, is_train=True, augment_train=True)
            if idx % 500 == 0:
                print(f"  processed {idx}/{len(train_selected)} train")
        train_count = idx
        print(f"  total train samples (with augmentation): {len(description_train)} desc + {len(grounding_train)} grd")
        for entry in test_selected:
            idx = process_patch(entry, idx, is_train=False, augment_train=False)
    ...
    # ---------- write JSONL files ----------
    output_files = [
        ("galamsey_v7_description_train.jsonl", description_train),
        ("galamsey_v7_description_eval.jsonl", description_eval),
        ("galamsey_v7_grounding_train.jsonl", grounding_train),
        ("galamsey_v7_grounding_eval.jsonl", grounding_eval),
        ("galamsey_v7_multitask_train.jsonl", multitask_train),
    ]
...
def main() -> None:
    ...
    for key, value in result.items():
        print(f"  {key}: {value}")
    print("\nNext step:")
    print("  uv run leap-finetune configs/galamsey_v7_modal.yaml")
