"""Batch-verify v9-e3 against earthrise-media's 685 Ghana mining sites.

For each earthrise polygon centroid:
  1. Fetch a 1.28 km Sentinel-2 RGB+SWIR tile from our Cloud Run SimSat
  2. Run v9-e3 inference
  3. Record whether v9 confirms or misses

Output: `/galamsey/earthrise_verification.json` with per-site status.

Usage:
    uv run modal run scripts/verify_earthrise_sites_modal.py
"""
...
app = modal.App("galamsey-verify-earthrise")
...
def verify_sites(sites_json: str) -> dict:
    ...
    print(f"Verifying {len(sites)} earthrise sites against v9-e3…")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    processor = AutoProcessor.from_pretrained(V9_CHECKPOINT, trust_remote_code=True)
    model = (
        AutoModelForImageTextToText.from_pretrained(
            V9_CHECKPOINT, torch_dtype=torch.bfloat16, trust_remote_code=True
        )
        .to(device)
        .eval()
    )
    ...
    with torch.no_grad():
        for i, s in enumerate(sites):
            ...
            if (i + 1) % 25 == 0:
                ...
                pathlib.Path("/galamsey/earthrise_verification.json").write_text(
    ...
    out_path = pathlib.Path("/galamsey/earthrise_verification.json")
...
def main():
    geojson_path = pathlib.Path("/tmp/ghana_mining.geojson")
    sites = _load_earthrise_centroids(geojson_path)
    print(f"Loaded {len(sites)} sites from {geojson_path}")
