"""v3 full-dataset prep, enriched descriptions + multi-task + single-pit oversampling.

Differences from prepare_v2_dataset_modal.py:

1. **Single-pit oversampling.** After the standard stratification, count how
   many training positives have pit_count == 1 vs pit_count >= 2. Duplicate
   single-pit positives SINGLE_PIT_DUPLICATION times (default 3) so the class
   balance within positives is roughly even. This addresses v2's failure mode
   identified in ARCHITECTURE.md §3.7.l where both v1 and v2 produced
   false-negative predictions on single-pit patches.

2. **Output path is `/galamsey/data/v3/`** so v2 data is preserved untouched.

Everything else (CSV parsing, stratification, enriched descriptions,
multi-task mix, seeded randomness) is identical to v2.

Usage:
    uv run modal run scripts/prepare_v3_dataset_modal.py
"""
...
MODAL_V3_DATA_DIR = f"{MODAL_MOUNT_POINT}/data/v3"
...
# v3 key change: duplicate each single-pit training positive this many times
# so the model sees enough "1 pit" examples to learn them as a distinct class.
# 3 × is based on the observed ratio in v2 (~30% single-pit positives) -
# duplication lifts their effective count to match the ~70% multi-pit positives.
SINGLE_PIT_DUPLICATION = 3

app = modal.App("galamsey-v3-prep")
...
def prepare() -> dict:
    ...
    # ---------- inlined primitives (self-contained for Modal) ----------
    ...
    data_dir = Path(MODAL_V3_DATA_DIR)
    images_dir = data_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("GalamseyWatch, v3 full dataset prep on Modal")
    print("=" * 60)
    print(f"RNG seed: {DATA_RNG_SEED}")
    print(f"Single-pit duplication factor: {SINGLE_PIT_DUPLICATION}×")

    print("\n[1/6] Downloading SmallMinesDS artifacts from HuggingFace...")
    ...
    print("\n[3/6] Scanning zip for image/mask pairs...")
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(
            f"  raw train: {len(train_pos)} pos + {len(train_neg)} neg  "
            f"test: {len(test_pos)} pos + {len(test_neg)} neg"
        )

        # ---------- single-pit oversampling ----------
        # Scan all training positives' masks to find single-pit ones.
        # This is O(train_positives) mask reads; acceptable on CPU.
        print(f"\n[4/6] Counting single-pit vs multi-pit training positives...")
        single_pit_train_pos = []
        multi_pit_train_pos = []
        for i, entry in enumerate(train_pos):
            if (i + 1) % 500 == 0:
                print(f"  scanned {i+1}/{len(train_pos)} positives")
            with zf.open(entry["mask_name"]) as fp:
                with rasterio.MemoryFile(fp.read()) as memfile:
                    with memfile.open() as src:
                        mask = src.read(1)
            _, n_pits = label(mask > 0)
            if n_pits == 1:
                single_pit_train_pos.append(entry)
            else:
                multi_pit_train_pos.append(entry)

        print(
            f"  single-pit train positives: {len(single_pit_train_pos)} "
            f"({100 * len(single_pit_train_pos) / len(train_pos):.1f}% of positives)"
        )
        print(f"  multi-pit train positives:  {len(multi_pit_train_pos)}")

        # Duplicate single-pit positives by SINGLE_PIT_DUPLICATION factor
        oversampled_single = single_pit_train_pos * SINGLE_PIT_DUPLICATION
        print(
            f"  after {SINGLE_PIT_DUPLICATION}× oversampling: "
            f"{len(oversampled_single)} single-pit training samples"
        )

        train_selected = oversampled_single + multi_pit_train_pos + train_neg
        stratify_rng.shuffle(train_selected)
        test_selected = test_pos + test_neg

        print(
            f"  final train: {len(train_selected)} samples "
            f"(was {len(train_pos) + len(train_neg)} in v2)"
        )

        # ---------- processing pass ----------
        print(f"\n[5/6] Processing {len(train_selected) + len(test_selected)} patches...")
    ...
    # ---------- multi-task JSONL ----------
    print("\n[6/6] Building multi-task training JSONL...")
    ...
    output_files = [
        ("galamsey_v3_description_train.jsonl", description_train),
        ("galamsey_v3_description_eval.jsonl", description_eval),
        ("galamsey_v3_grounding_train.jsonl", grounding_train),
        ("galamsey_v3_grounding_eval.jsonl", grounding_eval),
        ("galamsey_v3_multitask_train.jsonl", multitask_train),
    ]
    ...
    return {
        ...
        "single_pit_positives_raw": len(single_pit_train_pos),
        "single_pit_positives_after_oversample": len(oversampled_single),
        "multi_pit_positives": len(multi_pit_train_pos),
...
def main() -> None:
    print("Submitting v3 full dataset prep to Modal...")
    result = prepare.remote()
    print(f"\nPrep complete:")
    for key, value in result.items():
        print(f"  {key}: {value}")
    print("\nNext step:")
    print("  uv run leap-finetune configs/galamsey_v3_modal.yaml")
