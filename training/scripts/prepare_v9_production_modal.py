"""v9 production dataset prep, 450M WebGPU model, 4× augmentation, filtered bboxes.
...
Output path: `/galamsey/data/v9/`

Usage:
    uv run modal run scripts/prepare_v9_production_modal.py
"""
...
MODAL_V9_DATA_DIR = f"{MODAL_MOUNT_POINT}/data/v9"
...
app = modal.App("galamsey-v9-production-prep")
...
def prepare() -> dict:
    ...
    def analyze_mask(mask, min_pixels=MIN_PIT_PIXELS):
        ...
        for pid in range(1, n_pits + 1):
            ...
            if pixel_count < min_pixels:
                continue  # v9: skip tiny components
            ys, xs = np.where(pm)
            bboxes.append([
    ...
    # ---------- main ----------
    data_dir = Path(MODAL_V9_DATA_DIR)
    images_dir = data_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("GalamseyWatch, v9 production dataset (4× aug, filtered bboxes)")
    print("=" * 60)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        def process_patch(entry, idx, is_train):
            ...
            for aug_name, aug_fn in variants:
                ...
                rgb_name = f"v9_rgb_{new_idx:06d}.png"
                swir_name = f"v9_swir_{new_idx:06d}.png"
        ...
        print(f"\n  Processing train ({len(train_selected)} patches × 8 augmentations)...")
        idx = 0
        ...
        for name, samples in [
            ("galamsey_v9_description_train.jsonl", description_train),
            ("galamsey_v9_description_eval.jsonl", description_eval),
            ("galamsey_v9_grounding_train.jsonl", grounding_train),
            ("galamsey_v9_grounding_eval.jsonl", grounding_eval),
            ("galamsey_v9_multitask_train.jsonl", multitask_train),
        ]:
            path = data_dir / name
...
def main():
    print("Submitting v9 production dataset prep to Modal...")
