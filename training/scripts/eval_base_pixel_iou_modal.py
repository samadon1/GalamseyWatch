"""Base LFM2.5-VL-450M pixel-IoU eval, untrained baseline for the scoreboard.

Runs the raw HuggingFace checkpoint (no fine-tuning) against the full
SmallMinesDS test split using the same RGB+SWIR two-image prompt. The result
gives us the honest "before" number for the metrics modal on the landing page.

Usage:
    uv run modal run scripts/eval_base_pixel_iou_modal.py
"""
...
app = modal.App("galamsey-base-pixel-iou")
...
def run_pixel_iou_eval() -> dict:
    ...
    # ---------- Eval base HF model (no fine-tuning) ----------
    base_results = eval_checkpoint(f"base {BASE_MODEL_ID}", BASE_MODEL_ID)

    print("\n" + "=" * 72)
    print(f"BASE MODEL RESULTS ({BASE_MODEL_ID})")
    print("=" * 72)
    for metric in ["pixel_iou", "pixel_precision", "pixel_recall", "pixel_sdc_f1",
                   "pixel_accuracy", "mean_per_patch_iou", "patch_accuracy"]:
        print(f"  {metric:<25} {base_results[metric]:.4f}")
...
def main() -> None:
    print("Submitting base LFM2.5-VL-450M pixel-IoU eval to Modal...")
